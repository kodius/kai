...
`FormRadioGroup` wraps shadcn `RadioGroup` with react-hook-form `Controller`. It renders one horizontal `Field` per option from a `FormOption[]` array and exposes an optional `onSelectAction` callback that fires with the selected option.

File location: `components/form/form-radio-group/form-radio-group.tsx`
