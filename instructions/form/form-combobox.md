...
`FormCombobox` wraps shadcn `Popover` + `Command` with react-hook-form `Controller` to provide a searchable single-select. It supports a loading state, controlled or uncontrolled `open`, and uses `FormOption` for items.

It carries dual generics — `<T, TValue extends string = string>` — so the option `value` type can be narrowed to a string literal union when needed.

File location: `components/form/form-combobox/form-combobox.tsx`
